...
function decorateTemplateAst(ast) {
  ...
  var loopCount = 0;
  return transform(ast, function(obj) {
    if (obj.type == 'loop') {
      obj.objname = 'obj';
      obj.loopId = ++loopCount;
      obj.contents = obj.contents.map(function(stmt, i) {
        return transform(obj.contents[i], function(childobj) {
          var type = childobj.type;
          if (type == 'if' || type == 'unbound') {
            childobj.loopRef = loopCount;
          }
        });
      });
      --loopCount;
    ...
    if (Array.isArray(obj.path)) {
      obj.ensurePath = [];
      obj.objref = obj.loopRef ? obj.loopRef : 0;
...
function usesToTree(uses) {
  ...
  var findBlock = function(ast, block) {
    for (var i = 0; i < ast.length; i++) {
      if (ast[i].type == block.obj.type && ast[i].path == block.obj.path) {
        return ast[i];
      }
    }
...
if (require.main === module) {
  var assert = require('assert');
   ...
   // TODO next: some sort of tests for event listeners

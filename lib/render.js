...
function expandUses(tree, htmlElement) {
  ...
  function expandTemplate(node, uses, view) {
    var callbacks = uses.map(function(use, i) {
      ...
      var textBlobs = div.querySelectorAll('loop, not');
      _(textBlobs).forEach(function(textBlob) {
        while (textBlob.firstChild) {
          textBlob.parentNode.insertBefore(textBlob.firstChild, textBlob);
        }
        textBlob.parentNode.removeChild(textBlob);
      });

...
function dependenciesFor(tree) {
  ...
  deps = dependencies.flatten([[],
      tree.children().concat(deps).map(function(n) { return [n, []]; })]);
  ...
  // flattened trees.
  return dependencies.flatten([[], deps.map(function(d) {
    if (d.type == 'oak') {
      // We went through the oak dependencies above. No need to do it again.
      return [[], []];
    }
    return [[], tree.subtree(d).flatten().map(
        function(n) { return [n, []]; })];
  })]);
}
...
function widgetCreateJS(tree, expandedRoot) {
  var fileObjs = tree.flatten();
  var template = tree.data({ name: __dirname + '/create.mjs' });
...
function expandUses(fileObjs, htmlElement) {
  ...
  root.expandedUses = expandTemplate(htmlElement, root.uses, {});
  return root;
}
...
function html(tree, map, rapidId) {
  ...
  var fileObjs = tree.flatten();
  var expandedRoot = expandUses(fileObjs, html);
  ...
  if (rapidId) {
    var rapidJs = tree.data({ name: __dirname + '/rapid.mjs' }).data;

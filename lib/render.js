...
var mustacheOperate = dependencies.mustacheOperate;
...
/** Returns code that renders a mustache AST to var "result". */
function htmlJs(ast) {
  ...
  var transforms = {
    ...
    loop: function(ast, state) {
      ...
      return 'var arr = obj' + accessFor(path) +
             ...
             '  result += (function() {' +
             mustacheOperate(transforms, ast.contents, path) + '})(); }';
    },
  ...
  return 'var result = "";' +
         mustacheOperate(transforms, ast, []);
}
...
function html(tree, map, rapidId) {
  ...
  var expandedUses = dependencies.expandDOM(
      templates, html, root.uses, {}, mustache.to_html);

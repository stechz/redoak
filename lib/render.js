...
function dependenciesFor(tree) {
  ...
  dependencies.push.apply(dependencies, _(tree[1]).pluck('0'));
  dependencies = _(dependencies).uniq();

  // Take these dependencies and map them to their subtrees. Flatten out the subtrees
  // so that leaf nodes come first, then flatten out the list of flattened trees.
...
function html(tree) {
  ...
  for (var i = 0; i < cssFiles.length; i++) {
    ...
    el.setAttribute('href', cssFiles[i].servername);
  ...
  for (var i = 0; i < jsFiles.length; i++) {
    ...
    el.setAttribute('src', jsFiles[i].servername);

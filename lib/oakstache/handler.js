...
function stringifyPath(path) {
  if (path.type == 'i') {
    return 'i';
  } else if (path[0].backout) {
    var dotdot = new Array(path[0].backout + 1).join('../');
    return dotdot + path.slice(1).join('.');
  } else {
    return path.join('.');
  }
}
...
function stringifyOpenIf(ast) {
  var name = stringifyPath(ast.clause.path);
  ...
  if (ast.clause.type == 'simple') {
    ...
    result += 'not ' + name + ' is ' + rvalue;
  }
  return result + '}';
}

function stringifyOpenLoop(ast) {
  return '{loop ' + stringifyPath(ast.path) + '}';
}
    ...
    'if': function(ast, state) {
      var loopStateI = (function() {
        for (var i = state.length - 1; i >= 0; i--) {
          if (typeof parseInt(state[i]) == 'number') {
            return i;
          }
        }
        return -1;
      ...
      if (value && value.type == 'last') {
        var i = loopStateI();
        value = resolve(obj, state.slice(0, i)).length - 1;
      }
      ...
      if (clause.type == 'eq') {
        ...
        execute = isRealObject || bound !== false && forceStr(bound);
...
function renderStringUnbound(str, obj, escapefn) {
  var unresolved = function(ast, state) {
    if (ast.type == 'loop') {
      ...
      var name = stringifyPath(ast.path);
      if (ast.escape) {
        return '{' + name + '}';
      } else {
        return '{{' + name + '}}';
...
if (typeof require == 'function' && require.main == module) {
  ...
  (function test(str, json, result, state) {
    ...
    ('{loop .}{loop .}{.}{/}{/}', [[1,2],[3],[4]], '1234')

    // Loops with ifs
    ('{loop .}{if .i is .last}..{/}{.}{/}', [1, 2, 3], '12..3')

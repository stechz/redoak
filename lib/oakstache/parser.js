...
break;
case 7:
    oakstacheParse.tagStack.push({ openPath: $$[$0-1], openType: 'loop' });
    this.$ = null;
  
break;
case 8:
    oakstacheParse.tagStack.push({ openPath: $$[$0-1], openType: 'inverse' });
    this.$ = null;
...
case 9:
    ...
    while (tag = oakstacheParse.tagStack.pop()) {
      if (tag.openPath && $$[$0-1].join('.') == tag.openPath.join('.')) {
        stmts.reverse();
        this.$ = { type: tag.openType, path: $$[$0-1], contents: stmts };
        return;
      }
      stmts.push(tag);
    }
    this.$ = { type: 'endblock', openPath: $$[$0-1] };

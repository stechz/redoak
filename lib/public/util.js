...
util.TemplateStack = function(obj) {
  this.stack = [obj];
  this.lastName = '';
};

util.TemplateStack.prototype = {
  inverse: function(fn) {
    var cond = this.resolve();
    if (!cond) {
      fn();
    }
  },
  ...
  resolve: function() {
    for (var i = this.stack.length - 1; i >= 0; i--) {
      if (this.lastName !== '' &&
          this.stack[i] && this.stack[i][this.lastName]) {
          return this.stack[i][this.lastName];
      } else if (this.lastName === '' && this.stack[i]) {
        return this.stack[i];
      }
    }
    return undefined;
  ...
  push: function(name) {
    ...
    if (this.lastName !== '') {
      ...
      this.stack.push(obj ? obj[this.lastName] : undefined);
    }
    this.lastName = name;
  },

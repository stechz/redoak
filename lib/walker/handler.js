    ...
    if (unit.type == 'any') {
      ...
      var every = unit.filters.every(function(f) {
        if (f.type == 'has') {
          ...
          return (newobj[f.lhs] !== undefined && newobj[f.id] !== null &&
  ...
  for (var i = 0; i < selector.length; i++) {
    ...
    for (var j = 0; j <= fullpath.length; j++) {
      var haveMatch = (function decompose(part) {
        if (part.sep === '') {
          return unitMatches(fullpath.slice(0, j), part.unit);
        } else if (part.sep == '.') {
          if (!decompose(part.lhs)) {
            return false;
          }
          j++;
          return part.rhs == fullpath[j - 1];
        } else if (part.sep == '>') {
          if (!decompose(part.lhs)) {
            return false;
          }
          j++;
          return unitMatches(fullpath.slice(0, j), part.rhs);
        }
      })(part);
...
/*      if (part.type == 'simple' && unitMatches(path, part.unit)) {
        matches.push([i, j]);
      } else if (part.type == 'prop' && unitMatches(path, part.unit)) {
        var pathsMatch = (function() {
        ...
        j += part.path.length;
      } else if (part.type == 'gt') {
        var unitsMatch = (function() {
          for (var k = 0; k < part.units.length; k++) {
            if (!unitMatches(fullpath.slice(0, j + k), part.units[k])) {
              return false;
            }
          }
          return true;
        ...
        if (unitsMatch) {
          matches.push([i, j + part.units.length - 1]);
        }
        j += part.units.length;
      } */
    }
  ...
  if (DEBUG) {
    ...
    console.log('matches', matches.map(function(x) {
      return [x[0], x[1], fullpath.slice(0, x[1]).join('.')];
    }));
    console.log();
  }
...
function getSelectorScore(selector) {
  ...
  var getPartScore = function(part) {
    if (part.sep == '') {
      return getUnitScore(part.unit);
    } else if (part.sep == '>') {
      return getPartScore(part.lhs).concat(getUnitScore(part.rhs));
    } else if (part.sep == '.') {
      return getUnitScore(part.lhs);
    } else {
      return [-1];
    }
    var unit = part.unit;

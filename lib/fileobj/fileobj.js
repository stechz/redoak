...
/** Converts strings to fileobjs, and ensure fileobjs have types. */
function ensureFileObj(obj) {
  if (typeof obj == 'string') {
    obj = { filename: obj };
  }
  if (obj.filename && !obj.type) {
    obj = _.extend({}, obj, { type: path.extname(obj.filename).slice(1) });
  }
  return obj;
}
...
/** Like fetch, but synchronous. */
function fetchSync(files, fileObj) {
  fileObj = ensureFileObj(fileObj);
  if (files[fileObj.filename]) {
    return files[fileObj.filename];
  } else {
    try {
      var data = fs.readFileSync(fileObj.filename, 'utf-8');
      return handleFile(fileObj, data);
    } catch(e) {
      return null;
    }
  }

...
var fileHandlers = {
  ...
  'oak': function(fileObj, data) {
    ...
    // Gather up scripts and CSS.
    var fileObjs = [].concat(basicDependencies);
    var includes = document.querySelectorAll(
        'script[src], link[rel=js][href], link[rel=stylesheet][href]');
    ...
    // Gather up other oak files.
    _(document.querySelectorAll('link[rel=oak]')).map(function(link) {
      fileObjs.push({ name: link.getAttribute('href'), type: 'oak' });

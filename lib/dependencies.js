  ...
  parser.tokenizer.on('token', function listen(t) {
    if (t.type == 'Characters') {
      ...
      if (countDelta != 0) {
        ...
        if (!lastInserted || lastInserted.nodeType != lastInserted.TEXT_NODE ||
            lastInserted.nodeValue != t.data) {
      ...
      assert.ok(count > 0);
      openElements.last().oakLoopDepth = count;
    } else if (t.type == 'EOF') {
  ...
  'oak': function(fileObj, data) {
    ...
    _(document.querySelectorAll('template')).forEach(function(template) {
      ...
      var unbound = _(all).map(function(el, i) {
        if (el.tagName == 'USE' || el.oakLoopDepth > 0) {
          return;
        }
...
if (require.main === module) {
  ...
  assert.equal(table.rows[0].oakLoopDepth, 1);

  el = parseHTML('{{#loop1}}{{#loop2}}<div></div>{{/loop2}} ' +
                 '<div>etc</div>{{/loop1}}');
  var divs = el.querySelectorAll('div');
  assert.equal(divs.length, 2);
  assert.equal(divs[0].oakLoopDepth, 2);
  assert.equal(divs[1].oakLoopDepth, 1);

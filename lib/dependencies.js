...
html5.Parser.prototype.on('setup', function(parser) {
  var logics = [];
  var calcLogics = function() {
    return parser.tree.open_elements.filter(function(x) {
      return x.tagName == 'LOOP' || x.tagName == 'NOT';
    });
  };

  parser.tokenizer.on('token', function listen(t) {
    if (t.type == 'StartTag' &&
        (t.name.toLowerCase() == 'loop' || t.name.toLowerCase() == 'not')) {
      var prevOpen = parser.tree.open_elements.slice(-2, -1)[0];
      lastLoop = parser.tree.open_elements.last();
      prevOpen.appendChild(lastLoop);
      logics = calcLogics();
    } else if (t.type == 'StartTag' && logics.length > 0) {
      var newLoops = calcLogics();
      if (newLoops.length != logics.length) {
        assert.equal(newLoops.length + 1, logics.length);
        parser.tree.open_elements.splice(-1, 0, logics.last());
        logics.last().appendChild(parser.tree.open_elements.last());
      }
...
var fileHandlers = {
  ...
  'oak': function(fileObj, data) {
    // Wrap loops and nots in tags to make them easier to work with.
    data = data.replace(
        /{{#(.*?)}}([^]*?){{\/\1}}/g, '<loop>{{#$1}}$2{{/$1}}</loop>');
    data = data.replace(
        /{{\^(.*?)}}([^]*?){{\/\1}}/g, '<not>{{^$1}}$2{{/$1}}</not>');
    ...
    _(document.querySelectorAll('template')).forEach(function(template) {
      ...
      var logics = template.querySelectorAll('loop *, not *');
      all = _.intersect(all, logics);

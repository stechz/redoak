...
function expandDOM(templates, node, view, render) {
  ...
  var callbacks = _(node.querySelectorAll('use')).map(function(useEl, i) {
    ...
    return function() {
      ...
      if (el.tagName == 'USE') {
        // Remove use tag only after we have created a queryselector.
...
if (require.main === module) {
  ...
  var tree = buildForTesting(testData, function(tree) {
